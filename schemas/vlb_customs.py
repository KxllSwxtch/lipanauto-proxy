...
class TurnkeyPriceComponents(BaseModel):
    ...
    documents_fee: int = Field(default=105000, description="Broker Services fee in RUB (Услуги Брокера)")
    ...
    vladivostok_logistics_usd: int = Field(default=550, description="Vladivostok logistics in USD (legacy)")
    vladivostok_logistics_rub: int = Field(default=55000, description="Vladivostok logistics in RUB (Логистика до Владивостока)")
...
class VLBServiceConfig(BaseModel):
    ...
    cache_ttl_seconds: int = Field(default=0, description="Cache TTL (DISABLED - always fetch fresh)")
